...
- Get [WinDivert](https://github.com/basil00/WinDivert/releases)
    ...
    - Copy `include/windivert.h` to `netregu/include/`
    - Copy `x64/WinDivert.lib` to `netregu/lib/`
- Get [CLI11](https://github.com/CLIUtils/CLI11/releases)
    - Download the latest `CLI11.hpp` file and copy it to `netregu/include/`
- Build the app
    - Open a terminal and cd into the `netregu` folder
...
- Go back to the extracted WinDivert folder
    - Copy `x64/WinDivert.dll` to `netregu/build/x64-release/Release/`
    - Copy `x64/WinDivert64.sys` to `netregu/build/x64-release/Release/`


### How to use
- Run the app using a terminal with administrator privileges (required for WinDivert)
- Use command line options to specify which processes to limit and the desired bandwidth limits (`netregu --help` for details)
